...
indicadoresRouter.get("/", async (req, res) => {
  ...
  const where = somenteAtivos ? "WHERE i.ativo = true" : "";
  const result = await pool.query(`${SELECT_INDICADOR} ${where} ORDER BY e.ordem, d.ordem, i.ordem, i.nome`);
...
indicadoresRouter.post("/", async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO indicador (dimensao_id, nome, descricao, ordem, peso)
     VALUES ($1, $2, $3, COALESCE($4, 0), COALESCE($5, 1.0)) RETURNING id`,
    [dimensaoId, nome, descricao ?? null, ordem ?? null, peso ?? null]
  );
...
indicadoresRouter.put("/:id", async (req, res) => {
  ...
  await pool.query(
    `UPDATE indicador SET
     ...
     WHERE id = $7`,
    [dimensaoId ?? null, nome ?? null, descricao ?? null, ordem ?? null, peso ?? null, ativo ?? null, id]
  );
  const updated = await pool.query(`${SELECT_INDICADOR} WHERE i.id = $1`, [id]);
